...
export const MyProfilePage: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Informações do JWT para visualização */}
      {jwtPayload && (
        <Row className="mt-4">
          <Col md={12}>
            <Card>
              <Card.Header>
                <h5 className="mb-0">Informações do JWT (Apenas Visualização)</h5>
              </Card.Header>
              <Card.Body>
                <Row className="mb-3">
                  <Col sm={4} className="fw-bold">User UID:</Col>
                  <Col sm={8}>{jwtPayload.userUID}</Col>
                </Row>
                <Row className="mb-3">
                  <Col sm={4} className="fw-bold">Email (JWT):</Col>
                  <Col sm={8}>{jwtPayload.email}</Col>
                </Row>
                <Row className="mb-3">
                  <Col sm={4} className="fw-bold">Nome (JWT):</Col>
                  <Col sm={8}>{jwtPayload.name}</Col>
                </Row>
                <Row className="mb-3">
                  <Col sm={4} className="fw-bold">Função (JWT):</Col>
                  <Col sm={8}>{jwtPayload.role}</Col>
                </Row>
                <Row className="mb-3">
                  <Col sm={4} className="fw-bold">Expira em:</Col>
                  <Col sm={8}>{new Date(jwtPayload.exp * 1000).toLocaleString('pt-BR')}</Col>
                </Row>
              </Card.Body>
            </Card>
          </Col>
      ...
      <Row className="mt-4">
        <Col md={12}>
          <Card>
            ...
            <Card.Body>
              <Row className="mb-3">
                <Col sm={4} className="fw-bold">ID do Usuário:</Col>
                <Col sm={8}>{user.id}</Col>
              </Row>
              <Row className="mb-3">
                <Col sm={4} className="fw-bold">ID do Auth User:</Col>
                <Col sm={8}>{user.auth_user_id}</Col>
              </Row>
              <Row className="mb-3">
                <Col sm={4} className="fw-bold">Foto do Perfil:</Col>
                <Col sm={8}>{user.profile_picture_url || 'Não informado'}</Col>
              </Row>
              <Row className="mb-3">
                <Col sm={4} className="fw-bold">Redes Sociais:</Col>
                <Col sm={8}>
                  <pre className="mb-0" style={{ fontSize: '0.8rem' }}>
                    {JSON.stringify(user.social_media, null, 2)}
                  </pre>
                </Col>
              </Row>
              <Row className="mb-3">
                <Col sm={4} className="fw-bold">Criado em:</Col>
                <Col sm={8}>{new Date(user.created_at).toLocaleString('pt-BR')}</Col>
              </Row>
              <Row className="mb-3">
                <Col sm={4} className="fw-bold">Atualizado em:</Col>
                <Col sm={8}>{new Date(user.updated_at).toLocaleString('pt-BR')}</Col>
              </Row>
              {user.suspended_at && (
                <Row className="mb-3">
                  <Col sm={4} className="fw-bold">Suspenso em:</Col>
                  <Col sm={8}>{new Date(user.suspended_at).toLocaleString('pt-BR')}</Col>
                </Row>
              )}
            </Card.Body>
          </Card>
